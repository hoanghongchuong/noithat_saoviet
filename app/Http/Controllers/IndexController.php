...
class IndexController extends Controller {
	...
	*/
	...
	public function baogia()
	{
		...
		$title = 'Báo giá';
		$com = 'bao-gia';
		return view('templates.baogia',compact('data','title','com'));
	}
	public function phanphoi()
	{
		$data = DB::table('news')->where('status',1)->where('com','phan-phoi')->orderBy('id','desc')->get();
		$title = 'Phân phối sơn nước';
		$com = 'phan-phoi';
		return view('templates.phanphoi', compact('com','title','data'));
	}
	public function phanphoiDetail($alias)
	{
		$data = DB::table('news')->where('status',1)->where('alias',$alias)->first();

		$com='phan-phoi';
		$title = $data->title ? $data->title : $data->name;
		$description = $data->description;
		$keyword = $data->keyword;
		return view('templates.phanphoi_detail', compact('com','title','data'));
	}
	public function hoso()
	{
		$data = DB::table('about')->where('com','ho-so')->first();
		$title = 'Hồ sơ năng lực';
		$com = 'ho-so';
		return view('templates.hoso', compact('title', 'com','data'));
	}
	public function video()
	{
		...
		$com = 'video';
		$title = 'Video';
		return view('templates.video', compact('title','com','data'));
	}
	public function thicong()
	{
		$com = 'thi-cong';
		$title = 'Thi công';
		$data = DB::table('news')->where('status',1)->where('com','thi-cong')->orderBy('id','desc')->get();	
		return view('templates.thicong',compact('com','title','data'));
	}
	public function thicongDetail($alias)
	{
		$data = DB::table('news')->where('status',1)->where('alias',$alias)->first();
